...
```{r, message = T}
training(splits) %>% 
    plot_acf_diagnostics(purchase_at, .value = diff_vec(revenue))
```
...
```{r}
model_tbl_arima <- modeltime_table(model_fit_1_arima_basic,
    model_fit_2_arima_xregs,
    model_fit_3_arima_sarimax,
    model_fit_4_arima_sarimax,
    model_fit_5_arima_xreg_fourier) 
...
```{r}
model_tbl_exp_smooth %>% modeltime_calibrate(testing(splits))
```
